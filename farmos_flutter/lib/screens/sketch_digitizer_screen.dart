...
class SketchDigitizerScreen extends StatefulWidget {
  final int farmId;
  final Future<void> Function(String name, String soil, List<Offset> polygonPx,
      double areaM2, double areaAcres) onSave;
...
class _SketchDigitizerScreenState extends State<SketchDigitizerScreen> {
  ...
  Future<void> _runDetection(Uint8List bytes) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        setState(() {
          _error = 'Detection service returned an error (${response.statusCode}). '
      ...
      final rawEdges = (data['edges'] as List).map((e) => DetectedEdge(
        p1: Offset((e['x1'] as num).toDouble(), (e['y1'] as num).toDouble()),
        p2: Offset((e['x2'] as num).toDouble(), (e['y2'] as num).toDouble()),
      ...
      setState(() {
        ...
        _loading = false;
        if (rawEdges.isEmpty) {
          _error = 'No strong straight lines detected. Use "Add point manually" below instead.';
        }
      });
    } catch (e) {
      setState(() {
        _loading = false;
        _error = 'Could not reach the digitizer service at $_sketchServiceUrl.\n'
  ...
  void _askEdgeDistance(int index) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Edge distance',
  ...
  double _pixelAreaToM2(double pxArea) => pxArea * _scaleMetersPerPixel * _scaleMetersPerPixel;
  ...
  void _showFinalizeSheet() {
    ...
    if (poly.length < 3) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select at least 3 edges/points to form a shape')));
      return;
    }
    if (!_scaleSet) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tap an edge distance field and enter its real distance first')));
      return;
    }
            ...
            children: [
              ...
              Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                  Column(children: [
                    Text('${m2.round()} m²', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
                    Text('area', style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
                  ]),
                  Column(children: [
                    Text(acres.toStringAsFixed(3), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
                    Text('acres', style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
                  ]),
                  Column(children: [
                    Text('${poly.length}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
                    Text('points', style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
                  ]),
                ]),
              ),
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true, fillColor: const Color(0xFFF4F6F3),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                ),
              ),
              const SizedBox(height: 10),
              Wrap(spacing: 8, runSpacing: 6, children: soils.map((s) {
                final sel = soil == s;
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  await widget.onSave(nameCtrl.text.trim(), soil, poly, m2, acres);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Digitize Sketch',
          style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
      ),
      body: SafeArea(
        top: false,
              ...
              if (_imageBytes == null)
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.upload_rounded, size: 24, color: Color(0xFF1B4332)),
                        ),
                        const SizedBox(height: 10),
                        const Text('Upload Sketch', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
                      ],
                    ),
                  ),
                )
                ...
                Column(
                  ...
                  children: [
                    if (_loading)
                      const Padding(
                        ...
                        child: Center(child: Column(children: [
                          ...
                          SizedBox(height: 10),
                          Text('Detecting straight edges...', style: TextStyle(fontSize: 12, color: Colors.grey)),
                        ])),
                      ),
                    ...
                    if (!_loading)
                      LayoutBuilder(
                        builder: (ctx, constraints) {
                          ...
                          final displayH = displayW * (_imgH == 0 ? 0.6 : _imgH / _imgW);
                          ...
                          return Container(
                            ...
                            child: ClipRRect(
                              ...
                              child: Stack(
                                children: [
                                  Image.memory(_imageBytes!, width: displayW, height: displayH, fit: BoxFit.fill),
                                  GestureDetector(
                                    onTapDown: (d) {
                                      final imgPos = _displayToImageCoords(d.localPosition, displaySize);
                                      for (int i = 0; i < _edges.length; i++) {
                                        final mid = Offset((_edges[i].p1.dx + _edges[i].p2.dx) / 2,
                                                            (_edges[i].p1.dy + _edges[i].p2.dy) / 2);
                                    ...
                                    child: CustomPaint(
                                      ...
                                      painter: _OverlayPainter(
                                        ...
                                        toDisplay: (p) => _imageToDisplayCoords(p, displaySize),
                                      ),
                                    ),
                                  ),
                                ],
                    ...
                    const SizedBox(height: 10),
                    Text('Tap a colored line to toggle it on/off · Long-press to add a manual point (for curves)',
                      style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),

                    const SizedBox(height: 14),

                    if (_edges.isNotEmpty) ...[
                      const Text('Detected Edges', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1B4332))),
                      ...
                      ..._edges.asMap().entries.map((entry) {
                        ...
                        return Container(
                          ...
                          decoration: BoxDecoration(
                            color: e.selected ? const Color(0xFF52B788).withOpacity(0.08) : Colors.white,
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(color: e.selected ? const Color(0xFF52B788) : Colors.grey.shade200),
                          ),
                          child: Row(
                            children: [
                              Checkbox(
                              ...
                              Expanded(
                                child: Text('Edge ${i + 1} · confidence ${(e.confidence * 100).round()}%',
                                  style: const TextStyle(fontSize: 12, color: Color(0xFF1B4332))),
                              ),
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Text(
                                    e.distanceMeters != null ? '${e.distanceMeters!.toStringAsFixed(1)}m' : 'Set dist.',
                    ...
                    if (_manualPoints.isNotEmpty) ...[
                      const SizedBox(height: 10),
                      Text('${_manualPoints.length} manual point(s) added',
                    ...
                    Row(children: [
                      Expanded(
                        child: GestureDetector(
                          onTap: () => setState(() {
                            _imageBytes = null; _edges = []; _manualPoints = []; _error = null;
                          }),
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(color: Colors.grey.shade300),
                              borderRadius: BorderRadius.circular(12)),
                            child: Center(child: Text('Upload Different Sketch',
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 12),
                            decoration: BoxDecoration(color: const Color(0xFF1B4332), borderRadius: BorderRadius.circular(12)),
                            child: const Center(child: Text('Review & Save',
